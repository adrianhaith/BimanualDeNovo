...
subjNum = 2;
fname = fullfile(['BimanualSkillData_S',num2str(subjNum)])
load(fname)
%data = dat;

figure(80+subjNum)
plot([data.Bi{1}.pathlength data.Bi{2}.pathlength data.Bi{3}.pathlength data.Bi{4}.pathlength data.Bi{5}.pathlength]);
...
% figure out which trials were potential jump trials
    ...
    for i=1:300
        %plot(data.BiInit.Cr{i}(:,1),data.BiInit.Cr{i}(:,2),'b')
        if(clean(i))
            plot(data.Uni.Cr{i}(:,1),data.Uni.Cr{i}(:,2),'k')
        end
    end
    axis equal
    ylim([-.1 .2])
...
set(f,'Position',[200 200 1000 600])
set(f,'Color','w')
...
plot([1:size(dAll.Bi.pkVel,2)]+10,dAll.Bi.pkVel_1s(subjNum,:),'.-');
plot(1:size(dAll.Uni.pkVel,2),dAll.Uni.pkVel_1s(subjNum,:),'k.-');
%plot(dAll.pkVel(s,:),'.-','linewidth',2)
%plot(dAll.Uni.pkVel(s,:),'k.-','linewidth',2)
plot([0 45],mean(dAll.Uni.pkVel_1s(subjNum,:))*[1 1],'k')
plot(repmat([5.5 15.5 25.5]',1,2)+10,[0 1000],'k')
plot(repmat([10.5 20.5 30.5]',1,2)+10,[0 1000],'k:')
axis([0 45 0 .4])
xlabel('block')
...
%}
%%
f = figure(24); clf; hold on
set(f,'Position',[250 250 800 600])
set(f,'Color','w')

subplot(2,1,1); hold on
for i=1:data.Bi{1}.Ntrials
    plot(data.Bi{5}.tanVel{i},'b')
end
xlim([0 700])
ylim([0 .6])

subplot(2,1,2); hold on
for i=1:data.Uni.Ntrials
    plot(data.Uni.tanVel{i},'k')
end
xlim([0 700])
ylim([0 .6])
