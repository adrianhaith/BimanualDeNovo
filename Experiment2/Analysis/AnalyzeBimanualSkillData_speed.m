...
for subj = 1:length(d)
    ...
    for chunk=1:Nchunk
        ...
        dAll.Uni.pathLength_ratio(subj,:) = binData(d{subj}.Uni{6}.pathlength_null./d{subj}.Bi{chunk}{6}.pathlength,Nbin);
        ...
        dAll.Uni.pkVel_1s(subj,:) = binData(d{subj}.Uni{6}.pkVel_1s,Nbin);
        dAll.Uni.initDir(subj,:) = binData(d{subj}.Uni{6}.initDir,Nbin);
    ...
    % unimanual
        ...
        r = getResponses([-d{subj}.Uni{1}.CrX_post ; d{subj}.Uni{5}.CrX_post],.03,dt);
        dAll.Uni.response_large.pos(subj,:) = r.pos;
        dAll.Uni.response_large.vel(subj,:) = r.vel;
        dAll.Uni.response_large.vel_var(subj,:) = r.vel_var;
        
        r = getResponses([-d{subj}.Uni{2}.CrX_post ; d{subj}.Uni{4}.CrX_post],.015,dt);
        dAll.Uni.response_small.pos(subj,:) = r.pos;
        dAll.Uni.response_small.vel(subj,:) = r.vel;
        dAll.Uni.response_small.vel_var(subj,:) = r.vel_var;
        
        r = getResponses(d{subj}.Uni{3}.CrX_post,0,dt);
        dAll.Uni.response_nojmp.pos(subj,:) = r.pos;
        dAll.Uni.response_nojmp.vel(subj,:) = r.vel;
        dAll.Uni.response_nojmp.vel_var(subj,:) = r.vel_var;
    ...
    for c =1:Nchunk
        ...
        for pert = 1:5
            [dAll.Uni.peakResponse_large(subj), dAll.Uni.peakResponse_lat_large(subj)] = max(dAll.Uni.response_large.vel(subj,:));
            [dAll.Uni.peakResponse_small(subj), dAll.Uni.peakResponse_lat_small(subj)] = max(dAll.Uni.response_small.vel(subj,:));
            [dAll.Uni.peakResponse_nojmp(subj), dAll.Uni.peakResponse_lat_nojmp(subj)] = max(dAll.Uni.response_nojmp.vel(subj,:));
        end
...
save Bimanual_speed_compact dAll
